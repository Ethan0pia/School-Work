...
*****Author: Ethan Dunham
*****Date: 5/8/2017
*****Assignment: 261 Project 4
*****Description: This implements a BST.
************************************************************************************/
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	while (current != 0)
	{
		if (compare(val, current->val) == 0) return 1;
		else if (compare(current->val, val) > 0) current = current->left;
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	else if (compare(cur->val, val)>0) cur->left = _removeNode(cur->left, val);
