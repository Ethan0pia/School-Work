...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
	...
	//compare did not work correctly for this
...
void buildHeap(DynamicArray* heap, compareFunction compare)
{
	...
	while (first != -1)
	{
		adjustHeap(heap, heap->size, first, compare);
		--first;
	}
}
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
	...
	while (spot != 0) {
		par = (spot - 1) / 2;
		if (compare(dyGet(heap, spot), dyGet(heap, par))==-1)
		{
			dySwap(heap, par, spot);
			spot = par;
		}
		else {
			break;
...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
    // FIXME: implement
	assert(dySize(heap));
	int back = dySize(heap) - 1;
	dySwap(heap, back, 0);
	dyRemoveAt(heap, back);
	adjustHeap(heap, back, 0, compare);
}
...
void dyHeapSort(DynamicArray* heap, compareFunction compare)
{
	...
	int first = dySize(heap) / 2 - 1;

	for (int i = first; i > -1; i--) {
		adjustHeap(heap, dySize(heap), i, compare);
	}
	for (int i = dySize(heap) - 1; i > -1; i--) {
		dySwap(heap, 0, i);
